...
// membuat arrow function dengan nama variabel checkusers dan parameter users
const checkUsers = (users) => {
  // membuat sebuah variabel yang berisi users array kosong untuk menampung data dan membuat nilai total menjadi 0
  const result = {
    users: [],
    total: 0,
  };
  // membuat variabel looping dengan nama variabel nya user
  users.forEach((user) => {
    // membuat operator logika if
    if ((user.name.length < 10 && user.age > 30) || (user.eyeColor === "brown" && user.gender === "male")) {
      // masukkan data hasil dari operator logika ke dalam variabel array kosong yang telah dibuat
...
// panggil variabel checkuser dengan parameter users gunanya untuk mengecek data dari users
