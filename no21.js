...
const sortDescending = (array) => {
    ...
    do {
        ...
        // 2. Melakukan perulangan sebanyak panjang array
        for (let i = 0; i < array.length; i++) {
            // 3. Mengecek apakah array[i] lebih kecil daripada array[i+1]
            if (array[i] < array[i + 1]) {
                // 4. Melakukan swap nilai array[i] dengan index setelahnya jika nilai array[i] lebih kecil dari array[i+1]
                let temp = array[i];
                array[i] = array[i + 1];
                array[i + 1] = temp;
                // 5. Set nilai swapped ke true agar bisa melakukan pengecekan selanjutnya
